...
dashboard UIのxtermがresize/control sequenceを自動送信しうる場合は、専用の `/api/pty` WebSocket channelを開き、resizeを送らずに `ready` を待ってください。課題本文は1回だけ送信し、必要ならEnterのみを追加送信して確定してください。課題本文を再送する前に必ずSessions APIでuser messageを確認してください。
